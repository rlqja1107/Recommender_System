...
class CML(torch.nn.Module):
    ...
    def init_setting(self, train_set, test_set):
        ...
        self.train_set = torch.tensor(train_set, dtype=torch.long).cuda()
    ...
    def run(model):
        ...
        for epoch in range(model.epoch):
            ...
            print("epoch : {:d}, total_loss : {:.3f}, recall_50 : {:.4f}, recall_100 : {:.4f}, time : {:.4f}".format(
                epoch, total_loss, recall_50, recall_100, timer() - start))
